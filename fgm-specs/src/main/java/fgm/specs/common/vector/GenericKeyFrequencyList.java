...
import fgm.specs.data.implementation.vector.Record;
...
public class GenericKeyFrequencyList<IMPL extends Comparable<IMPL>> implements Vector {
        private ArrayList<AbstractMap.SimpleEntry<Record<IMPL>, Double>> stateVector;
        static Logger logger = Logger.getLogger(GenericKeyFrequencyList.class.getName());

        public GenericKeyFrequencyList(ArrayList<AbstractMap.SimpleEntry<Record<IMPL>, Double>> stateVector){
            PropertyConfigurator.configure(getClass().getClassLoader().getResourceAsStream("log4j.properties"));
            this.stateVector = stateVector;
        }
        public GenericKeyFrequencyList() {
            PropertyConfigurator.configure(getClass().getClassLoader().getResourceAsStream("log4j.properties"));
            this.stateVector = new ArrayList<>();
        }

        public ArrayList<AbstractMap.SimpleEntry<Record<IMPL>, Double>> getStateVector() {
            return this.stateVector;
        }

        public void setStateVector(ArrayList<AbstractMap.SimpleEntry<Record<IMPL>, Double>> stateVector) {
            this.stateVector = stateVector;
        }
        ...
        {
            Record<IMPL> record1 = (Record<IMPL>) record;
            boolean found = false;
            if(!this.stateVector.isEmpty()) {
                for(int i = 0; i < this.stateVector.size(); i++ ) {
                    if(record1.getKey().compareTo( this.stateVector.get(0).getKey().getKey() ) < 0 ) {
                        AbstractMap.SimpleEntry<Record<IMPL>, Double> recordChanged = new AbstractMap.SimpleEntry<>(record1, 1.0d);
        ...
        public void pack() {
            if(!this.stateVector.isEmpty()) {
                for (AbstractMap.SimpleEntry<Record<IMPL>, Double> integerRecord : this.stateVector) {
        ...
        public void sortStateVector() {
            ArrayList<AbstractMap.SimpleEntry<Record<IMPL>, Double>> newStateVector = new ArrayList<>(this.stateVector);
            if(!newStateVector.isEmpty()) {
                Collections.sort(newStateVector, new Comparator<AbstractMap.SimpleEntry<Record<IMPL>, Double>>() {
                    @Override
                    public int compare(AbstractMap.SimpleEntry<Record<IMPL>, Double> record1, AbstractMap.SimpleEntry<Record<IMPL>, Double> record2) {
        ...
        {
            GenericKeyFrequencyList<IMPL> keyFrequencyList = ((GenericKeyFrequencyList<IMPL>)(stateVector));

            ArrayList<AbstractMap.SimpleEntry<Record<IMPL>, Double>> newStateVector = new ArrayList<>();
            int i = 0, j = 0;
        ...
        public Vector subtract(Vector stateVector) {
            GenericKeyFrequencyList<IMPL> keyFrequencyList = ((GenericKeyFrequencyList<IMPL>)(stateVector));
            ArrayList<AbstractMap.SimpleEntry<Record<IMPL>, Double>> newStateVector = subtract(this, keyFrequencyList);
            return new GenericKeyFrequencyList<IMPL>(newStateVector);
        }


        private ArrayList<AbstractMap.SimpleEntry<Record<IMPL>, Double>> subtract(GenericKeyFrequencyList<IMPL> stateVector1, GenericKeyFrequencyList<IMPL> stateVector2) {
            ArrayList<AbstractMap.SimpleEntry<Record<IMPL>, Double>> newStateVector = new ArrayList<>();
            int i = 0;
            int j = 0;
            while(i < stateVector1.size() && j < stateVector2.size()) {
                if(stateVector1.getStateVector().get(i).getKey().getKey().compareTo(stateVector2.getStateVector().get(j).getKey().getKey()) < 0) {
                    newStateVector.add(new AbstractMap.SimpleEntry<Record<IMPL>, Double>(stateVector1.getStateVector().get(i).getKey(),  stateVector1.getStateVector().get(i).getValue()));
                    i++;
                }
                    ...
                    if(stateVector1.getStateVector().get(i).getValue() - stateVector2.getStateVector().get(j).getValue() > 0) {
                        newStateVector.add(new AbstractMap.SimpleEntry<Record<IMPL>, Double>(stateVector1.getStateVector().get(i).getKey(),  new Double(stateVector1.getStateVector().get(i).getValue() - stateVector2.getStateVector().get(j).getValue()) ));
                    }
                    i++;
                    j++;
                }
        ...
        public void scale(Double scalar) {
            for (AbstractMap.SimpleEntry<Record<IMPL>, Double> record : this.stateVector) {
