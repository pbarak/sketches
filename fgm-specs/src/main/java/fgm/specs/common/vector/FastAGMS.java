...
//@JsonDeserialize(using  = JsonInterfaceDeserializer.class)
public class FastAGMS implements Vector{
    static final long serialVersionUID = 1L; //assign a long value
    public final static long[][] test = init_test();
    ...
    IToStateVectorConvertor<?> convertor;


    public FastAGMS() {
        this.depth = 7;
        this.buckets = 5000;
        counts = initCounts(this.depth, this.buckets);
    }

    public FastAGMS(int depth, int buckets) {
        this.depth = depth;
        this.buckets = buckets;
        counts = initCounts(this.depth, this.buckets);
    }

    public FastAGMS(int depth, int buckets, IToStateVectorConvertor<?> convertor) {
        this.depth = depth;
        this.buckets = buckets;
        counts = initCounts(this.depth, this.buckets);
...
//    public void setCounts(FastAGMS fastAGMS) {
//        for (int i = 0; i < this.depth; i++) {
//            for (int j = 0; j < this.buckets; j++) {
//                this.counts[i][j] = fastAGMS.getSketchElement(i, j);
//            }
//        }
//    }
    ...
    public static long[][] init_test() {
        // the test[] array contains random numbers uniformly distributed in the range 0..2^31 which are
        // later used
    ...
    public int hash(int d, long x) {
        assert d < depth;
        return (int) (hash31(test[0][d], test[1][d], x) % buckets);
    }
    ...
    public int fourwise(int d, long x) {
        return (fourwise(test[2][d], test[3][d], test[4][d], test[5][d], x) > 0) ? 1 : -1;
    }

    private  IToStateVectorConvertor<Long> getIToStateVectorConvertor(){
        return (IToStateVectorConvertor<Long>) this.convertor;
    }

    public void update(StreamRecord streamRecord, int value) {
        Long key = convertor.toStateVectorRecord(streamRecord);
        for(int d = 0; d < depth; d++){
            if(streamRecord.getKey() instanceof Long) {
                int hash = hash(d, ( (Long) streamRecord.getKey() ).longValue());
                int xi = fourwise(d,  ( (Long) streamRecord.getKey() ).longValue());
                if(counts[d][hash] == null){
                    counts[d][hash] = Double.valueOf((double)value*xi);
                } else {
                    counts[d][hash] += Double.valueOf((double)value*xi);
                }
            }
    ...
    public Vector subtract(Vector sketch) {
        FastAGMS result = null;
        try {
            result = new FastAGMS(this.depth, this.buckets, DependencyInjection.getFactory().toStateVectorRecord());
        ...
        result.setCounts(this.getCounts());
        if(sketch instanceof  FastAGMS) {
            for (int i = 0; i < ((FastAGMS) sketch).depth(); i++) {
                for (int j = 0; j < ((FastAGMS) sketch).buckets(); j++) {
                    result.counts[i][j] = result.counts[i][j] - ((FastAGMS) sketch).getSketchElement(i, j);
                }
            }
        }
        return result;
    ...
    public void add(Vector fastAGMS) {
        if(fastAGMS instanceof FastAGMS) {
            for (int i = 0; i < ((FastAGMS)fastAGMS).depth(); i++) {
                for (int j = 0; j < ((FastAGMS)fastAGMS).buckets(); j++) {
                    if(counts[i][j] == null) {
                        counts[i][j] = ((FastAGMS)fastAGMS).getSketchElement(i, j);
                    } else {
                        counts[i][j] = counts[i][j] + ((FastAGMS) fastAGMS).getSketchElement(i, j);
                    }
                }
            }
        } else {
            throw new RuntimeException("Incompatible Vector Types");
        }
    }
...
//  public FastAGMS deepCopy() {
//    FastAGMS fastAGMS = this.newInstance(); //to-do create deep copy with serdes
//    double[][] newCounts = new double[this.depth][this.buckets];
//    for(int i = 0; i < this.depth; i++) {
//      if (this.buckets >= 0)
//        System.arraycopy(counts[i], 0, newCounts[i], 0, this.buckets);
//    }
//    fastAGMS.setCounts(newCounts);
//    return fastAGMS;
//  }

@JsonIgnore
    public Double[] getSketchColumn(int w) {
        Double[] column = new Double[this.buckets];
        System.arraycopy(this.counts[w], 0, column, 0, this.buckets);
        return column;
    }
    ...
    @JsonIgnore
    public void setSketchElement(int i, int j, Double value) {
        counts[i][j] = value;
    }

    public void scale(Double scalar) {
        Double[][] newCounts = initCounts(this.depth, this.buckets);


        for(int i = 0; i < this.depth; ++i) {
            for(int j = 0; j < this.buckets; ++j) {
                if(newCounts[i][j] != null) {
                    newCounts[i][j] = this.counts[i][j] / scalar;
                } else {
                    newCounts[i][j] = 0.0;
                }
            }
    ...
    private Double[][] initCounts(int depth, int buckets){
        Double[][] initCounts = new Double[depth][buckets];
